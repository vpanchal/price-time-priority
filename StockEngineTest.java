...
public class StockEngineTest {
	...
	public static void main(String args[]) {
		...
		HashMap<OrderAttributes,OrderType> orderAttribute = new HashMap<>();
		
		//Sample test Data
		orderAttribute.put(new OrderAttributes(OrderType.BUY, 300, 25, 123456L, 14), OrderType.BUY);
		orderAttribute.put(new OrderAttributes(OrderType.SELL, 400, 25, 123426L, 13), OrderType.SELL);
		orderAttribute.put(new OrderAttributes(OrderType.BUY, 220, 25, 123453L, 15), OrderType.BUY);
		orderAttribute.put(new OrderAttributes(OrderType.SELL, 800, 25, 123426L, 17), OrderType.SELL);
		...
		try {
			...
			for (int i = 0; i < orders.size(); i++) {
				...
				if (orders.get(i).contains(OrderType.SELL.name())) {
					sells.add(orders.get(i));
				}

				if (orders.get(i).contains(OrderType.BUY.name())) {
...
//To test the program pass user input as below

//order 300 sell at 32 
//order 200 buy at 25
